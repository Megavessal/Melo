// Melo/Views/HUD/HUDWindowController.swift
import AppKit
import SwiftUI
import os
...
final class HUDWindowController: MediaKeyHUDPresenting {
    ...
    var frameProvider: () -> NSRect? = { HUDWindowController.activeScreenVisibleFrame() }
    ...
    /// Displays the HUD. Skipped only when the popup is already visible.
    ///
    /// There used to be a fullscreen guard here, and it produced the worst
    /// possible outcome: the media-key monitor had already swallowed the
    /// keypress, so suppressing our own HUD left a fullscreen video with *no*
    /// volume feedback at all — the one case where the system HUD always
    /// shows. The panel's `.fullScreenAuxiliary` collection behaviour lets it
    /// draw over fullscreen spaces, so there is nothing to guard against.
    ...
    func showPerAppNotControlledHUD(displayName: String?, bundleID: String?, icon: NSImage?) {
        let title = displayName?.nilIfEmpty
            ?? bundleID?.nilIfEmpty
            ?? "Melo isn't controlling this app yet"
        presentPerApp(
            icon: icon,
            title: title,
    ...
    /// Hotkey-triggered per-app HUDs; suppressed only while the popup is open.
    ...
    private func ensurePanel() -> NSPanel {
        ...
        // Above `.floating`: a fullscreen app's window sits at the normal
        // level but its space is composited on top of ordinary floating
        // panels, so the HUD has to sit in the same band as the menu bar and
        // the system's own volume HUD to stay visible over fullscreen video.
        p.level = .statusBar
    ...
    private func postPerAppAccessibilityAnnouncement(panel: NSPanel, title: String, content: PerAppHUDContent) {
        ...
        case .notControlled:
            description = "\(title), not controlled by Melo"
        }
        NSAccessibility.post(
            element: panel,
            notification: .announcementRequested,
    ...
    /// Visible frame of the display the user is actually looking at.
    ///
    /// `NSScreen.main` is the screen owning the *key window*, which on a
    /// multi-display setup is wherever focus last landed — frequently not the
    /// display showing the video whose volume just changed. The pointer is the
    /// better proxy, with `.main` kept as the fallback for the case where the
    /// pointer is off every screen.
    static func activeScreenVisibleFrame() -> NSRect? {
        let mouse = NSEvent.mouseLocation
        if let underPointer = NSScreen.screens.first(where: { $0.frame.contains(mouse) }) {
            return underPointer.visibleFrame
        }
...
private struct PerAppHUD: View {
    ...
    /// Matches TahoeStyleHUD.shape — continuous, not circular.
    private static let shape = DesignTokens.Dimensions.Shape.custom(cornerRadius)
    private static let percentageWidth: CGFloat = 36
    private static let iconSize: CGFloat = 28
    private static let barHeight: CGFloat = 4

    private var subtitleText: String? {
        if case .notControlled = content { return "Not controlled by Melo" }
        return nil
    }
    ...
    private var accessibilityDescription: String {
        ...
        case .notControlled:
            return "\(title), not controlled by Melo"
        }
    }
    ...
    private var iconView: some View {
        if let icon {
            ...
            Image(systemName: "app.dashed")
                .font(DesignTokens.Typography.Scale.title2(.regular))
    ...
    private var trailingRow: some View {
        ...
        case .volume:
            HStack(spacing: DesignTokens.Spacing.sm) {
                ...
                Text(percentageText)
                    .font(DesignTokens.Typography.Scale.footnote(.semibold).monospacedDigit())
                    .foregroundStyle(DesignTokens.Colors.textSecondary)
