// Melo/Views/Settings/Components/ThemeTilePicker.swift
import SwiftUI
...
// MARK: - Melo visual theme picker

@MainActor
struct MeloVisualThemePicker: View {
    @Binding var selection: MeloVisualTheme

    var body: some View {
        HStack(spacing: 8) {
            ForEach(MeloVisualTheme.allCases) { theme in
                Button {
                    selection = theme
                } label: {
                    VStack(spacing: 5) {
                        ZStack {
                            RoundedRectangle(cornerRadius: 7, style: .continuous)
                                .fill(previewGradient(for: theme))
                        ...
                        .overlay {
                            RoundedRectangle(cornerRadius: 7, style: .continuous)
                                .strokeBorder(
                                    selection == theme ? Color.accentColor : Color(nsColor: .separatorColor),
                                    lineWidth: selection == theme ? 2 : 0.5
                                )
                        }

                        Text(theme.description)
                            .font(.system(size: 10, weight: selection == theme ? .semibold : .regular))
                            .foregroundStyle(selection == theme ? .primary : .secondary)
                    }
                }
                .buttonStyle(.meloHover)
                .accessibilityLabel("\(theme.description) theme")
                .accessibilityAddTraits(selection == theme ? .isSelected : [])
            }
        }
    }

    private func previewGradient(for theme: MeloVisualTheme) -> LinearGradient {
        switch theme {
        case .systemAccent:
            return LinearGradient(
                colors: [Color(nsColor: .controlAccentColor), Color(nsColor: .controlAccentColor).opacity(0.45)],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
        case .space:
            return LinearGradient(
                colors: [.blue.opacity(0.85), Color(red: 0.05, green: 0.02, blue: 0.15)],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
        case .galaxy:
        ...
        case .aurora:
            return LinearGradient(
                colors: [Color(red: 0.02, green: 0.08, blue: 0.18), .green.opacity(0.85), .cyan.opacity(0.65)],
