// Melo/Views/Rows/DeviceRow.swift
import SwiftUI
...
struct DeviceRow: View {
    ...
    private var deviceHeader: some View {
        HStack(spacing: DesignTokens.Spacing.sm) {
            ...
            LiquidGlassSlider(
                value: $sliderValue,
                accessibilityTitle: "\(device.name) volume",
...
extension View {
    /// Draws the keyboard-selection ring for a row.
    ///
    /// Rows used to signal keyboard selection by reusing `hoverableRow`'s
    /// hover fill, which made "the pointer is over this row" and "this row is
    /// what the arrow keys will act on" pixel-identical — with the pointer
    /// resting anywhere in the popup, keyboard navigation was invisible. An
    /// accent stroke is a separate channel, and matches the focus affordance
    /// Full Keyboard Access draws elsewhere on the system.
