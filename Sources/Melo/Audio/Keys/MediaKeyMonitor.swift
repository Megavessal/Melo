// Melo/Audio/Keys/MediaKeyMonitor.swift
import AppKit
import AudioToolbox
import CoreGraphics
import os
...
final class MediaKeyMonitor {
    ...
    private let logger = Logger(subsystem: "io.github.megavessal.Melo", category: "MediaKeyMonitor")
    ...
    /// can flash the current device's transport symbol. Wired by MeloApp after init.
    var iconCoordinator: MediaKeyIconFlashing?

    /// Plays the system volume-feedback pop on volume key steps. Wired by MeloApp after init.
    ...
    func handle(_ event: MediaKeyEvent, shiftHeld: Bool = false, optionHeld: Bool = false) {
        ...
        handleCore(
            ...
            setVolume: { id, vol in self.audioEngine.setOutputDeviceVolume(for: id, to: vol) },
            setMute:   { id, mute in self.audioEngine.setOutputDeviceMute(for: id, to: mute) },
