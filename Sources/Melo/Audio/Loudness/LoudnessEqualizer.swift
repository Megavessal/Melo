...
final class LoudnessEqualizer: @unchecked Sendable {
    ...
    private let adaptiveProcessor: AdaptiveAudioProcessor
    ...
    init(settings: LoudnessEqualizerSettings, sampleRate: Float) {
        ...
        self.adaptiveProcessor = AdaptiveAudioProcessor(settings: settings.adaptiveAudio, sampleRate: sampleRate)
    ...
    func process(
        ...
        if adaptiveProcessor.isEnabled {
        ...
        } else if input != UnsafePointer(output) {
            memcpy(output, input, frameCount * channelCount * MemoryLayout<Float>.size)
        }

        guard settings.widebandLevelingEnabled else { return }
        ...
        if channelCount == 2 {
            for frame in 0..<frameCount {
                let base = frame * 2
                let mono = (output[base] + output[base + 1]) * 0.5
                ...
                output[base] *= linearGain
                output[base + 1] *= linearGain
            }
            return
        }
