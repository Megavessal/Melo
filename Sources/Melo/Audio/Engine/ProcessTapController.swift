// Melo/Audio/Engine/ProcessTapController.swift
import AudioToolbox
import Foundation
import os
...
final class ProcessTapController: ProcessTapControlling {
    ...
    /// Lock-free Stereo Field targets, plus callback-owned smoothing state.
    /// Primary and secondary values remain independent because route crossfades
    /// run concurrently on separate HAL I/O threads.
    private nonisolated(unsafe) var _primaryStereoFieldEnabled: Bool = false
    private nonisolated(unsafe) var _primaryStereoFieldPosition: Float = 0
    private nonisolated(unsafe) var _primaryStereoFieldLeftGain: Float = 1
    private nonisolated(unsafe) var _primaryStereoFieldRightGain: Float = 1
    private nonisolated(unsafe) var _secondaryStereoFieldEnabled: Bool = false
    ...
    /// Independent per-app and all-captured-app Audio Unit chains for the two HAL callbacks.
    /// Atomic slots publish retained unmanaged pointers and defer destruction until their realtime
    /// readers quiesce. A chain is never shared by callbacks that can overlap.
    private nonisolated let primaryAppAudioUnitSlot = RealtimeAudioUnitChainSlot()
    private nonisolated let secondaryAppAudioUnitSlot = RealtimeAudioUnitChainSlot()
    private nonisolated let primarySystemAudioUnitSlot = RealtimeAudioUnitChainSlot()
    private nonisolated let secondarySystemAudioUnitSlot = RealtimeAudioUnitChainSlot()

    /// Main-actor format bookkeeping used by AudioEngine to build plug-ins away from the IOProc.
    private(set) var audioUnitRenderConfiguration: AudioUnitRenderConfiguration?
    private var secondaryAudioUnitRenderConfiguration: AudioUnitRenderConfiguration?
    private var appAudioUnitRuntimeIsCurrent = false
    private var systemAudioUnitRuntimeIsCurrent = false
    var needsAudioUnitRuntimeRebuild: Bool {
        !appAudioUnitRuntimeIsCurrent || !systemAudioUnitRuntimeIsCurrent
    }
    ...
    init(
        ...
        self.logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Melo", category: "ProcessTapController(\(app.name))")
    }
    ...
    func updateStereoFieldSettings(_ settings: StereoFieldSettings) {
        ...
        _primaryStereoFieldEnabled = normalized.isEnabled
        _primaryStereoFieldPosition = normalized.position
        _secondaryStereoFieldEnabled = normalized.isEnabled
    ...
    /// Publishes a freshly-instantiated pair to the realtime callbacks. Each atomic slot retires
    /// its previous +1 only after all readers have left, so no plug-in construction, ARC, or
    /// destruction occurs in the IOProc.
    func installAudioUnitRuntimePair(
        _ pair: AudioUnitRuntimePair,
        scope: AudioUnitProcessingScope
    ) {
        guard let primaryConfiguration = audioUnitRenderConfiguration,
              pair.primary.isCompatible(with: primaryConfiguration) else {
            markAudioUnitRuntimeStale(scope)
            return
        }

        // Do not publish an old-format secondary while a crossfade is live. AudioEngine will
        // schedule another build after promotion using the new primary format.
        if secondaryResources.isActive,
           let secondaryConfiguration = secondaryAudioUnitRenderConfiguration,
           !pair.secondary.isCompatible(with: secondaryConfiguration) {
            markAudioUnitRuntimeStale(scope)
            return
        }
        ...
        switch scope {
        case .app:
            primaryAppAudioUnitSlot.replace(with: pair.primary)
            secondaryAppAudioUnitSlot.replace(with: pair.secondary)
            appAudioUnitRuntimeIsCurrent = true
        case .system:
            primarySystemAudioUnitSlot.replace(with: pair.primary)
            secondarySystemAudioUnitSlot.replace(with: pair.secondary)
            systemAudioUnitRuntimeIsCurrent = true
        }
    }

    private func markAudioUnitRuntimeStale(_ scope: AudioUnitProcessingScope) {
        switch scope {
        case .app: appAudioUnitRuntimeIsCurrent = false
        case .system: systemAudioUnitRuntimeIsCurrent = false
        }
    }

    // MARK: - Multi-Device Aggregate Configuration

    /// Resolved plan for Melo's private wrapping aggregate: which hardware sub-devices
    ...
    private func buildAggregateDescription(outputUIDs: [String], tapUUID: UUID, name: String) -> [String: Any] {
        ...
        let plan = Self.planAggregate(
            ...
            expand: { uid in
                ...
                // be flattened into Melo's aggregate (nested aggregates aren't supported).
    ...
    /// Reads a conservative maximum slice size from the private aggregate. This happens during
    /// setup only; the value is then exposed to AudioEngine for off-thread plug-in construction.
    private func audioUnitConfiguration(
        for aggregateDeviceID: AudioObjectID,
        fallbackSampleRate: Double = 48_000
    ) -> AudioUnitRenderConfiguration {
        let sampleRate = (try? aggregateDeviceID.readNominalSampleRate()) ?? fallbackSampleRate
        let currentFrames: UInt32 = (try? aggregateDeviceID.read(
            kAudioDevicePropertyBufferFrameSize,
            defaultValue: UInt32(512)
        )) ?? 512
        let fallbackMaximum = max(currentFrames, 4_096)
        let range: AudioValueRange = (try? aggregateDeviceID.read(
            kAudioDevicePropertyBufferFrameSizeRange,
            defaultValue: AudioValueRange(
                mMinimum: Double(currentFrames),
                mMaximum: Double(fallbackMaximum)
            )
        )) ?? AudioValueRange(
            mMinimum: Double(currentFrames),
            mMaximum: Double(fallbackMaximum)
        )
        let finiteMaximum = range.mMaximum.isFinite
            ? min(Double(UInt32.max), max(Double(currentFrames), range.mMaximum))
            : Double(fallbackMaximum)
        return AudioUnitRenderConfiguration(
            sampleRate: sampleRate,
            maximumFrames: max(1, UInt32(finiteMaximum.rounded(.up)))
        )
    }

    private func preparePrimaryAudioUnitRuntimes(
        for configuration: AudioUnitRenderConfiguration
    ) {
        audioUnitRenderConfiguration = configuration

        if let chain = primaryAppAudioUnitSlot.currentValue(),
           !chain.isCompatible(with: configuration) {
            primaryAppAudioUnitSlot.replace(with: nil)
        }
        if let chain = secondaryAppAudioUnitSlot.currentValue(),
           !chain.isCompatible(with: configuration) {
            secondaryAppAudioUnitSlot.replace(with: nil)
        }
        appAudioUnitRuntimeIsCurrent = primaryAppAudioUnitSlot.hasValue
            && secondaryAppAudioUnitSlot.hasValue

        if let chain = primarySystemAudioUnitSlot.currentValue(),
           !chain.isCompatible(with: configuration) {
            primarySystemAudioUnitSlot.replace(with: nil)
        }
        if let chain = secondarySystemAudioUnitSlot.currentValue(),
           !chain.isCompatible(with: configuration) {
            secondarySystemAudioUnitSlot.replace(with: nil)
        }
        systemAudioUnitRuntimeIsCurrent = primarySystemAudioUnitSlot.hasValue
            && secondarySystemAudioUnitSlot.hasValue
    }

    private func prepareSecondaryAudioUnitRuntimes(
        for configuration: AudioUnitRenderConfiguration
    ) {
        secondaryAudioUnitRenderConfiguration = configuration

        if let chain = secondaryAppAudioUnitSlot.currentValue(),
           !chain.isCompatible(with: configuration) {
            secondaryAppAudioUnitSlot.replace(with: nil)
            appAudioUnitRuntimeIsCurrent = false
        }
        if let chain = secondarySystemAudioUnitSlot.currentValue(),
           !chain.isCompatible(with: configuration) {
            secondarySystemAudioUnitSlot.replace(with: nil)
            systemAudioUnitRuntimeIsCurrent = false
        }
    }

    /// Promotes the secondary chains and keeps the old primary copies ready for the next route
    /// crossfade when their stream format is still compatible.
    private func promoteAudioUnitRuntimes(
        to configuration: AudioUnitRenderConfiguration
    ) {
        let oldPrimaryApp = primaryAppAudioUnitSlot.currentValue()
        let promotedApp = secondaryAppAudioUnitSlot.currentValue()
        if let promotedApp, promotedApp.isCompatible(with: configuration) {
            primaryAppAudioUnitSlot.replace(with: promotedApp)
        } else {
            primaryAppAudioUnitSlot.replace(with: nil)
        }
        if let oldPrimaryApp, oldPrimaryApp.isCompatible(with: configuration) {
            secondaryAppAudioUnitSlot.replace(with: oldPrimaryApp)
        } else {
            secondaryAppAudioUnitSlot.replace(with: nil)
        }
        appAudioUnitRuntimeIsCurrent = primaryAppAudioUnitSlot.hasValue
        ...
        let oldPrimarySystem = primarySystemAudioUnitSlot.currentValue()
        let promotedSystem = secondarySystemAudioUnitSlot.currentValue()
        if let promotedSystem, promotedSystem.isCompatible(with: configuration) {
            primarySystemAudioUnitSlot.replace(with: promotedSystem)
        } else {
            primarySystemAudioUnitSlot.replace(with: nil)
        }
        if let oldPrimarySystem, oldPrimarySystem.isCompatible(with: configuration) {
            secondarySystemAudioUnitSlot.replace(with: oldPrimarySystem)
        } else {
            secondarySystemAudioUnitSlot.replace(with: nil)
        }
        systemAudioUnitRuntimeIsCurrent = primarySystemAudioUnitSlot.hasValue
        ...
        audioUnitRenderConfiguration = configuration
        secondaryAudioUnitRenderConfiguration = nil
    }
    ...
    private struct AudioDeviceStartRequest: @unchecked Sendable {
        let deviceID: AudioObjectID
        let procID: AudioDeviceIOProcID
    }

    private nonisolated static func startAudioDeviceOffMainThread(
        _ request: AudioDeviceStartRequest
    ) async -> OSStatus {
        await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                continuation.resume(
                    returning: AudioDeviceStart(request.deviceID, request.procID)
                )
            }
        }
    }
    ...
    func activate(initial: TapInitialState) throws {
        let request = try prepareActivation(initial: initial)
        let err = AudioDeviceStart(request.deviceID, request.procID)
        try finishActivation(startStatus: err)
    }

    /// Used only by the first-run audio primer. Core Audio may wait while macOS
    /// presents its system-audio permission sheet. Running that single blocking
    /// C call on a worker queue keeps SwiftUI, the permission window, and Finder
    /// responsive. All resource creation and state mutation remain @MainActor.
    func activateWithoutBlockingMainThread(initial: TapInitialState) async throws {
        let request = try prepareActivation(initial: initial)
        let err = await Self.startAudioDeviceOffMainThread(request)
        try finishActivation(startStatus: err)
    }

    private func prepareActivation(initial: TapInitialState) throws -> AudioDeviceStartRequest {
        guard !activated else {
            guard let procID = primaryResources.deviceProcID else {
                throw NSError(
                    domain: "ProcessTapController",
                    code: -2,
                    userInfo: [NSLocalizedDescriptionKey: "Tap is active but its IO process is unavailable"]
                )
            }
            return AudioDeviceStartRequest(
                deviceID: primaryResources.aggregateDeviceID,
                procID: procID
            )
        }
        ...
        preparePrimaryAudioUnitRuntimes(
            for: audioUnitConfiguration(
                for: primaryResources.aggregateDeviceID,
        ...
        let initialStereoField = initial.stereoFieldSettings.normalized
        let initialStereoGains = initialStereoField.channelGains
        _primaryStereoFieldEnabled = initialStereoField.isEnabled
        ...
        err = AudioDeviceCreateIOProcIDWithBlock(&primaryResources.deviceProcID, primaryResources.aggregateDeviceID, queue) { @Sendable [weak self] _, inInputData, _, outOutputData, outputTime in
            guard let self else {
        ...
        guard let procID = primaryResources.deviceProcID else {
            ...
            throw NSError(
                domain: "ProcessTapController",
                code: -3,
                userInfo: [NSLocalizedDescriptionKey: "Core Audio did not create an IO process"]
            )
        }
    ...
    private func finishActivation(startStatus err: OSStatus) throws {
        guard err == noErr else {
            cleanupPartialActivation()
    ...
    private func endInvalidation() {
        ...
        primarySystemAudioUnitSlot.replace(with: nil)
        secondarySystemAudioUnitSlot.replace(with: nil)
        audioUnitRenderConfiguration = nil
        secondaryAudioUnitRenderConfiguration = nil
        appAudioUnitRuntimeIsCurrent = false
        systemAudioUnitRuntimeIsCurrent = false
    ...
    private func createSecondaryTap(for outputUIDs: [String]) throws {
        ...
        prepareSecondaryAudioUnitRuntimes(
            for: audioUnitConfiguration(
                for: secondaryResources.aggregateDeviceID,
                fallbackSampleRate: sampleRate
            )
        )
        ...
        err = AudioDeviceCreateIOProcIDWithBlock(&secondaryResources.deviceProcID, secondaryResources.aggregateDeviceID, queue) { @Sendable [weak self] _, inInputData, _, outOutputData, outputTime in
            guard let self else {
    ...
    private func destroyPrimaryTap() {
        // Secondary audio continues while this synchronous teardown waits for any in-flight
        // primary callback. That gives processor and Audio Unit promotion a hard lifetime barrier.
    ...
    private func promoteSecondaryToPrimary() {
        let promotedAudioUnitConfiguration = secondaryAudioUnitRenderConfiguration
            ?? audioUnitConfiguration(for: secondaryResources.aggregateDeviceID)
        ...
        _primaryStereoFieldEnabled = _secondaryStereoFieldEnabled
        ...
        // The barrier ensures the HAL I/O thread sees the updated EQ processors, Audio Units,
        // volume, ramp coefficient, stereo channels, and Stereo Field state BEFORE it sees the
        // new _primaryCallbackID and switches to primary-role behavior.
    ...
    private func performDeviceSwitch(to outputUIDs: [String]) throws {
        ...
        preparePrimaryAudioUnitRuntimes(
            for: audioUnitConfiguration(for: newResources.aggregateDeviceID)
        )
        secondaryAudioUnitRenderConfiguration = nil
        ...
        err = AudioDeviceCreateIOProcIDWithBlock(&newResources.deviceProcID, newResources.aggregateDeviceID, queue) { @Sendable [weak self] _, inInputData, _, outOutputData, outputTime in
            guard let self else {
                // Zero output to prevent garbage audio if controller is deallocated
                let outputs = UnsafeMutableAudioBufferListPointer(outOutputData)
                for buf in outputs {
    ...
    @inline(__always)
    nonisolated private static func processMappedBuffersRT(
        ...
        appAudioUnitChain: Unmanaged<StereoAudioUnitEffectChain>?,
        systemAudioUnitChain: Unmanaged<StereoAudioUnitEffectChain>?,
        ...
        let safeStereoPosition = stereoFieldPosition.isFinite
            ? min(1, max(-1, stereoFieldPosition))
            : 0
        let targetStereoLeftGain: Float
        let targetStereoRightGain: Float
        if !stereoFieldEnabled {
            targetStereoLeftGain = 1
            targetStereoRightGain = 1
        } else if safeStereoPosition < 0 {
            targetStereoLeftGain = 1
            targetStereoRightGain = 1 + safeStereoPosition
        } else {
            targetStereoLeftGain = 1 - safeStereoPosition
            targetStereoRightGain = 1
        }
        ...
        for outputIndex in 0..<outputBufferCount {
            ...
            // Per-app Stereo Field is the final tonal stage, immediately before
            // limiting. Gains are smoothed independently for this callback path so
            // toggles and balance changes cannot click, including during crossfades.
            if outputChannels > 1 && safeLeft != safeRight {
                for frame in 0..<frameCount {
                    stereoFieldLeftGain += (targetStereoLeftGain - stereoFieldLeftGain) * rampCoefficient
                    stereoFieldRightGain += (targetStereoRightGain - stereoFieldRightGain) * rampCoefficient
                    let base = frame * outputChannels
                    outputSamples[base + safeLeft] *= stereoFieldLeftGain
                    outputSamples[base + safeRight] *= stereoFieldRightGain
                }
            } else {
                // Keep smoothing state current even for a temporarily mono/unknown
                // channel layout, without attenuating its single channel.
            ...
            // User-ordered Audio Units run after Melo's built-in EQ and Stereo Field, with the
            // app chain feeding the all-captured-app chain. These unmanaged values are borrowed
            // under their slots' reader counts, so the RT path performs no retain/release.
            if eqCanProcessStereoInterleaved {
                var renderTimestamp = audioTimestamp
                if renderTimestamp.mFlags.isEmpty {
                    renderTimestamp.mHostTime = mach_absolute_time()
                    renderTimestamp.mFlags = .hostTimeValid
                }
                if let appAudioUnitChain {
                    _ = appAudioUnitChain.takeUnretainedValue().processInterleavedInPlace(
                        outputSamples,
                        frameCount: UInt32(frameCount),
                        timestamp: &renderTimestamp
                    )
                }
                if let systemAudioUnitChain {
                    _ = systemAudioUnitChain.takeUnretainedValue().processInterleavedInPlace(
                        outputSamples,
                        frameCount: UInt32(frameCount),
                        timestamp: &renderTimestamp
                    )
    ...
    /// Compatibility overload for deterministic pipeline tests. Live HAL callbacks use
    /// `processMappedBuffersRT` with reader-protected unmanaged references.
    @inline(__always)
    nonisolated static func processMappedBuffers(
        ...
        loudnessEqualizerProc: LoudnessEqualizer?,
        loudnessCompensatorProc: LoudnessCompensator?,
        appAudioUnitChain: StereoAudioUnitEffectChain? = nil,
        ...
        processMappedBuffersRT(
            ...
            stereoFieldEnabled: stereoFieldEnabled,
            ...
            appAudioUnitChain: appAudioUnitChain.map(Unmanaged.passUnretained),
            systemAudioUnitChain: systemAudioUnitChain.map(Unmanaged.passUnretained),
            audioTimestamp: audioTimestamp
        )
    }

    /// Compatibility overload for pipeline tests and callers that do not apply
    /// Stereo Field processing. The live callback uses the stateful overload above.
    @inline(__always)
    nonisolated static func processMappedBuffers(
        inputBuffers: UnsafeMutableAudioBufferListPointer,
        outputBuffers: UnsafeMutableAudioBufferListPointer,
        ...
        processMappedBuffers(
            ...
            stereoFieldPosition: 0,
            stereoFieldLeftGain: &leftGain,
            stereoFieldRightGain: &rightGain,
            eqProc: eqProc,
            autoEQProc: autoEQProc,
            loudnessEqualizerProc: loudnessEqualizerProc,
            loudnessCompensatorProc: loudnessCompensatorProc
    ...
    nonisolated private func processAudioCallback(
        ...
        let appAudioUnitChain: Unmanaged<StereoAudioUnitEffectChain>?
        let systemAudioUnitChain: Unmanaged<StereoAudioUnitEffectChain>?
        ...
        if isPrimary {
            ...
            stereoFieldEnabled = _primaryStereoFieldEnabled
            ...
            loudnessCompensatorProc = loudnessCompensator
            appAudioUnitChain = primaryAppAudioUnitSlot.beginRead()
            systemAudioUnitChain = primarySystemAudioUnitSlot.beginRead()
        } else {
            ...
            stereoFieldEnabled = _secondaryStereoFieldEnabled
            ...
            appAudioUnitChain = secondaryAppAudioUnitSlot.beginRead()
            systemAudioUnitChain = secondarySystemAudioUnitSlot.beginRead()
        }

        Self.processMappedBuffersRT(
            inputBuffers: inputBuffers,
            outputBuffers: outputBuffers,
            targetVol: targetVol,
            crossfadeMultiplier: crossfadeMultiplier,
            ...
            loudnessEqualizerProc: loudnessEqualizerProc,
            loudnessCompensatorProc: loudnessCompensatorProc,
            appAudioUnitChain: appAudioUnitChain,
            systemAudioUnitChain: systemAudioUnitChain,
            audioTimestamp: timestamp
        )

        // Balance both beginRead calls even when a slot held nil. Releasing the reader counts may
        // make retired chains eligible for main-actor destruction, but performs no ARC itself.
        if isPrimary {
            primarySystemAudioUnitSlot.endRead()
            primaryAppAudioUnitSlot.endRead()
        } else {
            secondarySystemAudioUnitSlot.endRead()
            secondaryAppAudioUnitSlot.endRead()
        }
